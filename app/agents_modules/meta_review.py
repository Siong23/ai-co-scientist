        ...
        *,
        ...
        if proximity_data:
            ...
            if n_clusters == 1 and len(active_hypotheses) > 2:
                ...
                cluster_names = [
                    cluster_labels.get(c_id, {}).get("label", f"Cluster {c_id + 1}")
                    for c_id in list(clusters.keys())[:4]
                ]
                comment_summary.append(
            ...
            if outliers:
                comment_summary.append(
                    ...
                    "worth investigating or off-topic noise — review them manually: "
                    + ", ".join(outliers[:5])
                    + "."
                )
                next_steps.append(
        ...
        if not comment_summary:
            comment_summary.append(
                "Overall hypothesis quality seems reasonable based on automated review."
            )

        # ----------------------------------------------------------------
        # Top-ranked hypotheses
        # ----------------------------------------------------------------
        best_hypotheses = sorted(
            active_hypotheses, key=lambda h: h.elo_score, reverse=True
        )[:3]
        _legacy.logger.info(
        ...
        if context.meta_review_feedback:
            ...
            prev_steps = (
                (prev.get("research_overview") or {}).get("suggested_next_steps") or []
            )
            if prev_steps:
                next_steps.append(
                    f"[Continuing from prior cycle] {prev_steps[0]}"
                )
