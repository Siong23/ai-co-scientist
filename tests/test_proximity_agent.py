...
from app.agents_modules.proximity import (
    ProximityAgent,
...
def test_proximity_agent_multi_hypotheses_and_clustering():
    ...
    with patch("app.agents_modules.proximity._legacy.similarity_score", side_effect=mock_sim):
        result = agent.build_proximity_graph(context, run_near_duplicate_check=False)
...
def test_proximity_agent_caching():
    ...
    with patch("app.agents_modules.proximity._legacy.similarity_score", sim_mock):
...
def test_proximity_agent_empty_text_handling():
    ...
    with patch("app.agents_modules.proximity._legacy.similarity_score", return_value=0.5):
        result = agent.build_proximity_graph(context, run_near_duplicate_check=False)
...
def test_near_duplicate_detected():
    ...
    with (
        patch("app.agents_modules.proximity._legacy.similarity_score", side_effect=mock_sim),
        patch("app.agents_modules.proximity._legacy.call_llm", return_value=llm_response),
    ):
...
def test_near_duplicate_rejected_by_low_confidence():
    ...
    with (
        patch("app.agents_modules.proximity._legacy.similarity_score", return_value=0.80),
        patch("app.agents_modules.proximity._legacy.call_llm", return_value=llm_response),
    ):
        result = agent.build_proximity_graph(context, run_near_duplicate_check=True)
...
def test_near_duplicate_check_skipped_when_disabled():
    ...
    with (
        patch("app.agents_modules.proximity._legacy.similarity_score", return_value=0.99),
        patch("app.agents_modules.proximity._legacy.call_llm", llm_mock),
    ):
        result = agent.build_proximity_graph(context, run_near_duplicate_check=False)
...
def test_cluster_label_generated():
    ...
    with (
        patch("app.agents_modules.proximity._legacy.similarity_score", return_value=0.85),
        patch("app.agents_modules.proximity._legacy.call_llm", return_value=label_response),
    ):
        result = agent.build_proximity_graph(context, run_near_duplicate_check=False)
...
def test_cluster_label_fallback_on_llm_error():
    ...
    with (
        patch("app.agents_modules.proximity._legacy.similarity_score", return_value=0.85),
        patch("app.agents_modules.proximity._legacy.call_llm", return_value="not valid json"),
    ):
        result = agent.build_proximity_graph(context, run_near_duplicate_check=False)
...
def test_call_llm_for_cluster_label_ok():
    ...
    with patch("app.agents_modules.proximity._legacy.call_llm", return_value=response):
...
def test_call_llm_for_cluster_label_fallback():
    with patch("app.agents_modules.proximity._legacy.call_llm", return_value="garbage"):
...
def test_call_llm_for_near_duplicate_check_fallback_on_bad_json():
    ...
    with patch("app.agents_modules.proximity._legacy.call_llm", return_value="not json at all"):
        result = call_llm_for_near_duplicate_check(h_a, h_b)
    assert result["near_duplicate"] is False
